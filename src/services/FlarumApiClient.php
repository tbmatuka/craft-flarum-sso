...
class FlarumApiClient
{
    ...
    public function createAccount(array $userDetails)
    {
        ...
        if (isset($userDetails['nickname']) && $userDetails['nickname'] != '') {
            $attributes['nickname'] = $userDetails['nickname'];
        }
    ...
    /**
     * Change an account nickname in Flarum
     *
     * @param array $user ['username' => string, 'email' => string, 'password' => string]
     * @param int|null $flarumUserId
     *
     * @return  array ['error' => bool, 'data' => ?array|string, 'errors' => ?array]
     */
    public function updateUser(array $user, ?int $flarumUserId = null)
    {
        if ($flarumUserId == null) {
